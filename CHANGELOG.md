...
- **#621 — Interactive-draft conformance floors: deterministic output floors for `outline` and `draft` phases.** Two content-addressed conformance rules (`OUTLINE_NO_HTML_RULE`, `DRAFT_SELF_CHECK_RULE`) are upserted at arm time and pinned to their respective phases via `validator_pin`. The outline floor rejects HTML markup (`<html>`, `<!DOCTYPE>`, `<body>`) and output exceeding 40 000 characters; the draft floor requires the `wicked-garden-draft` self-check to PASS (stamped only when the published snapshot holds the skill). A failed upsert is non-fatal — the phase proceeds ungated with a log warning, preserving pre-#621 behavior. `adapter.upsertConformanceRule` now returns `Promise<string>` (the content-address hash) instead of `Promise<void>`.
