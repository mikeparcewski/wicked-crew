...
### Added
- **Seam X2 — one answer to "what is this run?", from the engine's record (`wicked-crew-api-types` 0.46.0).** `core/run-identity.ts` resolves a run from its plan state (`AgentSession.team_plan.preset`, else a user plan), else the launch's recorded `sessionStarted.workflowId` in the engine's event log (read once per run, memoized), else a non-synthetic `workflow_id` — never from its phase sequence. A preset run whose floor added steps now resolves to its preset for the acceptance routes, the steering-author landing, test-set registration, delivery candidacy and the deliver text; a `<run>:plan-<rev>` run resolves as a user plan; a non-team registered workflow still resolves. The phase-sequence matching in `resolveRunWorkflow` and in `sessionsDetail()` is deleted (`resolveRunWorkflow` is gone; callers use `runWorkflowDef` / `runNameOf`). Served runs carry `AgentSession.run_identity {kind, name, user_plan, system}`; `team_plan`, `WorkUnit.catalog` and `WorkUnit.team_run` (engine fields already on the wire) are typed. The system workflows live in ONE list (`SYSTEM_WORKFLOWS`, keyed by preset or workflow name) that stamps `WorkflowDef.is_system` on `GET /workflows` and sets `run_identity.system`; the per-def `is_system` literals are gone. **Acceptance and delivery read what the run CONTAINS:** a preset or user-plan run declares acceptance on its units whose catalog entry carries `verified_evidence` (read from `Core.catalog()`, `CatalogEntry.verified_evidence`), never a registered def looked up by name, so a user plan with a `test` step requires acceptance and a `feature` preset run keeps its requirement when the `feature` def is deleted. A non-team registered workflow still reads its def. What cannot be read fails CLOSED: declared and denied, with the reason in the gate (`the run's identity is unknown`; plan steps without catalog ids; an engine catalog without the flag; a workflow no longer registered). Delivery candidacy of a plan run is its units' code work (`executes_code`, not the evaluator's). `tests/run-identity.test.ts` pins it through the real engine.
- **DES-TEAMING-002 T8 — the engine bindings wired (wicked-core#630; `wicked-crew-api-types` 0.45.0).** `GET /catalog` and `POST /plans/preview` call `Core.catalog` / `Core.previewPlan` (501 only on an addon without the method). `POST /plans/preview` takes the launch's `repoRef` (its code graph scores the `touch` set; an unregistered repo is a 400) and `deliver` (`"pr"` hands the engine the launch's deliver step, so the preview's floor carries `deliver` as the launch's would; a plan launch defaults to no delivery). **`POST /runs/:id/plan` mid-run** (running, or paused at a gate that is not a `plan_approval`) calls `Core.proposePlan(id, plan, requestId)`: the steps to ADD, held for the next step boundary and approved by their author (no gate opens), answered with the engine's `{proposal_id, duplicate, band, high_risk, floor_added}` and audited as `plan.edit.proposed`. `requestId` stays optional: omitted, the daemon mints a fresh UUID for that POST, so only a caller-sent id makes a retry idempotent (`duplicate: true`). The engine's refusals (a plan awaiting approval — edit it at the gate —, a started deliver step, a finished or unplanned run, an edit carrying `touch`/`override`) answer 409 with its reason; at a `plan_approval` gate the edit is still the gate answer (`confirmGate … edit_plan`, `{status}`). api-types: `CatalogEntry` and `PlanPreviewResponse` repinned to the engine's exact shapes (`added_by_floor` removed: the floor's additions are the `steps` with `added_by: "floor"`; `score`, `deterministic`, `reasons`, `destructive`, `floor`, `floor_override`, `def`, `pauses`, `pause_reason`, `graph` added), `PlanPreviewBody.repoRef`/`deliver`, `PlanProposalResponse`, `EditPlanGateResponse`, `EditPlanResponse`. `tests/team-engine.test.ts` pins all three against the real engine.
- **DES-TEAMING-002 T8 — the crew team surface (commands only, §4.0).** Crew relays, reads and POSTs; it publishes no `wicked.team.*` row (`tests/team-no-publish.test.ts` fails the build on a team emit). **Relay:** every `wicked.team.*` row the engine publishes reaches `/ws` as `{type:"teamEvent", event}`, tagged `project_id` when the run's membership files it (`src/team/ws-relay.ts`, the `interactiveEvent` pattern; armed only when the engine has a bus). **`GET /runs/:id/team`:** the engine's persisted team state (`Core.runTeam`: `transport` `bus`/`none`/`pending`/`unavailable` with its `reason`; per unit `ledgerSource`, `ledgerRef`, `finalPass`, `teamPause`, `findings`) joined with the run's bus rows by `event_id` and the folded ledger each gate read; a `ledger.folded` row whose gate named another ledger is labelled `unused`; a run that is not a team run answers `teamed: false`, `transport: null`, `units: []` and `ended` from its status (never the `"none"` outage, which means a teamed run fell back); with the rows gone the snapshot still answers. **Commands:** `POST /runs/:id/plan {plan}` is the plan edit at a `plan_approval` gate (the engine publishes `plan.proposed{by:"human", kind:"edit"}` once; audited as `gate.decided {action:"edit_plan"}`; off a `plan_approval` gate it answers 501, since a mid-run edit needs the engine's `proposePlan`; an optional `requestId` is accepted for it); `POST /runs {plan, deliver:"pr"}` now hands the engine the deliver step (T3 refused it); `POST /team/outbox/replay` runs `Core.replayTeamOutbox`; the `team_transport` answers (approve, approve with amend `"continue without team"`, reject) and `team_dispute` approve go through `POST /runs/:id/gate` unchanged. **`GET /catalog`, `POST /plans/preview`:** served from the engine's `Core.catalog` / `Core.previewPlan`; an addon without them (every core-ts to date) answers 501. The state-home fixture mirrors core's `team-outbox.ndjson` entry.

- **DES-TEAMING-002 T3 — the plan approval gate, crew's commands (wicked-core#622).** `POST /runs` accepts a user-composed `plan` (`{steps, touch?, override?}`, `LaunchRunBody.plan`; a step's `id` defaults to its catalog id) and forwards it to the engine as `LaunchOptions.planJson` — the ENGINE publishes `plan.proposed{by:"human"}`, scores it (a creator plan with no `touch` scores 100, "no declared scope"), floor-fills it and holds it at a `plan_approval` gate when the approval matrix says so (always in manual mode; in auto mode when high risk). `POST /runs/:id/gate` answers that gate: `{approve:true}` releases the held plan, `{approve:true, plan}` (or `action:"edit_plan"`) approves WITH an edited plan (accepted as the next rev with floor phases added, or refused and the gate re-opened), `{approve:false}` rejects. `plan` is exclusive with `workflow`; `deliver:"pr"` with a plan answers 400 until T8 wires delivery for plans. Both paths fail CLOSED with a 501 on an addon without `Core.supportsPlanLaunch` (napi drops undeclared fields, which would run the launch unplanned and ungated). Crew publishes no `wicked.team.*` row. **With an engine carrying T3, a preset launch is floor-filled and gated too** (`tests/preset-routes.test.ts` pins both shapes). **A delivering preset launch no longer composes `<preset>-deliver-<run>`:** that def is no preset, so the DEFAULT launch (repo-scoped `feature`, `deliver:"pr"`) skipped `plan_approval`. Crew now launches the preset itself and hands the engine its deliver step (`LaunchOptions.deliverStepJson`, the same Tool command, gate text and evidence-floor pin; `verified_evidence` is `false` per DES-TEAMING-002 rev 12 note 4), with no composed fallback (501 on an addon without the gate). `requireDeliverables` on a preset launch is refused (the floor has no engine-side step yet). Registered defs (`bug`, `migration`, drop-ins) keep the per-run composition until their migration seams make them presets.

### Changed
- **X1 — the PA scopes a plan that declares no scope (wicked-core#633; `wicked-crew-api-types` 0.47.0).** With an engine carrying X1, a creator plan with no `touch` and every creator preset run first run the PA's read-only `pa-scope` step: rev 1 is `[pa-scope]` (unit ord 1, accepted by the engine at launch), the scored plan is rev 2 proposed by the PA seat (`plan.proposed{by:<PA seat>, kind:"initial", base_rev:1}`), and its `plan_approval` gate, when one opens, is at ord 2. A plan that declares `touch` is still scored at launch (rev 1, gate at ord 1). **Acceptance fails closed while the PA scopes:** a preset or user-plan run whose `team_plan.scope` is set has only `pa-scope` as a unit, so reading its units declared no requirement (a gate that passes with nothing required); it is now declared and denied with "the run's plan is still being scoped by its PA". **Delivery candidacy** treats such a run like one with no planned units yet (a candidate), never as a run with no code work. api-types: `PlanPreviewResponse.graph` gains `"pending_pa_scope"` (a no-`touch` creator plan previews at the baseline's score and band with `pa-scope` first, and `pauses` is manual mode's alone), `TeamPlanState.scope` (`TeamPlanScopeHold`) is typed, and `LaunchRunBody.plan` documents the scope step. Real-engine tests: gate-mechanics tests declare `touch`; preset and no-touch tests wait for the scoped plan and expect `pa-scope` first; `team-engine` pins the PA's proposal on a teamed rig (its `pa-scope` unit waits the engine's 30 s `MIN_GATE_WAIT`, with `WICKED_TEAM_FINAL_PASS_SECS=1`) and the `pending_pa_scope` preview.
- **Crew writes and reads the bus through the engine; the daemon loads no SQLite library (wicked-core#631).** crew#680 moved in-daemon bus writes to a child-process writer and reads to one long-lived better-sqlite3 handle, leaving two SQLite libraries in the daemon on the engine's bus file. Now every seam emits with the engine's `Core.busEmit` and reads with `Core.busRead` (`src/core/bus.ts`): the adapter attaches its engine to the bus file it handed it, and a seam whose bus no engine holds does not arm (it logs why) — no fallback. **Deleted:** `core/bus-writer.ts` (the child writer, its NDJSON protocol and idle timer), `core/bus-handle.ts` and its never-close rule, `core/bus-tap.ts`, the WB-002 branches (a duplicate key now resolves to the existing row), and wicked-bus as a runtime dependency (now a test dependency; `wicked-ledger` is imported from its SQLite-free `manifest` subpath). The boot probe is a plain file open, not a SQLite connection. `GET /runs/:id/team` and the project activity feed read their rows through the engine as history — rows past the bus TTL included (`busRead` `includeExpired`), since nothing sweeps the daemon's bus. **Removed: the QE gate bus tap** (`serve --qe-gate-events` / `WICKED_QE_GATE_EVENTS`, `src/qe/gate-events.ts`, and the acceptance view's `busEvent` field): it read the daemon's crew bus while garden's QE emitters write the HOME wicked-bus, so it never fired; `GET /runs/:id/acceptance` reads the QE ledger, unchanged. An engine without `Core.busEmit`/`busRead` now also says so on `/health.warnings` (`bus.seams_off`), not only on the console. `tests/bus-no-write.test.ts` now fails the build if importing any in-daemon module loads better-sqlite3 or `node:sqlite`, or any in-daemon source names a SQLite library or wicked-bus as a loadable module. Needs a wicked-core-ts with `Core.busEmit`/`busRead`; with an older one the bus seams stay off.
- **DES-TEAMING-002 T0: one daemon, one bus file.** The engine is now handed the daemon's own cross-product bus (`resolveCrewBus`: `--bus-db` › `WICKED_BUS_DATA_DIR` › `<core db>.bus/bus.db`) as `WICKED_BUS_DB` on every boot, not only under `--engine-exec`. `--engine-exec` still sets `WICKED_BUS_EXEC` and now mediates over that same file; its old crew-private `<state home>/bus.db` default is gone. The ready line's `busDb` reports the handed bus on every boot. At boot crew opens one long-lived crew bus handle (`core/bus-handle.ts`) before the engine spawns, and the project activity feed's per-request open/close goes through it. Each SQLite library in the process now holds its bus connection for the life of the process (the F-E2E-021 lock-loss class). A bus that cannot open logs one `[crew] bus unavailable` line, hands the engine no bus, and raises a `bus.unavailable` notice on `/health.warnings`; the per-run `transport:"none"` lands with P1. An engine without `Core.busConnectionStats` (it predates the one-connection rule) gets the pre-T0 handoff, with one line saying so. An unavailable bus dominates every branch (`core/engine-bus.ts`): whichever file the engine would get — including the pre-T0 exec bus — if it cannot open, the engine gets no bus and `--engine-exec` is off. An engine that could not arm its bus bridge within its bound (core-ts `busBridgeState()` = `not-armed`) is reported on the same `bus.unavailable` notice. `process.env` matches the handoff exactly before the engine spawns (an inherited `WICKED_BUS_DB`/`WICKED_BUS_EXEC` the decision did not hand is removed), and the crew bus handle is reused only while the path names the file it opened (`dev:ino`); a recreated file gets a fresh handle. The `bus.unavailable` notice words each failure kind: `probe_open` (crew could not open the file: fix the path or its permissions) and `bridge_not_armed` (the engine could not arm its bus bridge: find what holds the bus locked or slow).

### Changed (wire, additive)
- **`POST /runs/:id/gate` names the gate it answers (`wicked-crew-api-types` 0.44.0).** An optional `GateDecision.ord` — the unit `ord` of the gate the decision was made on. When present the daemon resolves the run's open gate the way `GET /runs/:id/gate` serves it (cache › the engine's durable `interaction_requests` row › the event-log replay, now one shared `resolveOpenGate`) and answers **409** `{ error: "Gate changed: …", code: "gate_changed", openOrd }` on a mismatch, confirming and auditing nothing — a skin's queued decision (studio's 10 s undo window) can no longer answer a gate the person never saw. When the daemon cannot tell which gate is open (no cache, no durable row, no log binding, or a log recording no open gate), a decision carrying `ord` is refused with **409** `{ code: "gate_unknown" }` — "refresh and decide again"; no `ord` = today's behaviour. The `gate.decided` audit detail carries the `ord`. `tests/gate-ord-route.test.ts` pins it.
- **`GET /audit?since=` — the trail since an instant (`wicked-crew-api-types` 0.43.0).** An inclusive lower bound on the entry clock (`ts >= since`, unix millis), composing with `?runId=` / `?action=` and applied before `?limit=`; a malformed value answers 400 like `limit`. The read a skin's "while you were away" handover makes, `system` actors' entries (the stall watchdog's `run.stall.*`) included. `AuditQuery` names the four filters; the endpoint manifest now declares the route's `AuditPage` response and its 200/400/500 codes. `tests/audit-since-route.test.ts` pins it.
- **DES-TEAMING-002 T8 — the team surface wire contract (`wicked-crew-api-types` 0.42.0).** `TeamEventFrame`, `TeamRow`, `RunTeamUnit`, `RunTeamResponse` (`teamed`; `transport` null when not teamed), `EditPlanBody` (`requestId?`), `TeamOutboxReplayReport`, `CatalogEntry`/`CatalogResponse`, `PlanPreviewBody`/`PlanPreviewResponse`; T6's ledger fields `TeamLedger.stepReviews` (`TeamStepReview`), `TeamLedgerFinding.anchor` and `carriedFromAttempt`. `LaunchRunBody.plan` documents `deliver:"pr"`.
- **DES-TEAMING-002 T1 — the team wire contract (`wicked-crew-api-types` 0.41.0).** Additive types for
  the 25 `wicked.team.*` bus events wicked-core declares in `src/team/events.rs` (wicked-core#616):
  `TeamEventType`, `TeamEnvelope`, one `Team*Payload` per type, `TeamLedger`, the `TeamEventPayloads`
  map and the `TeamBusEvent` union. Crew publishes none of them (DES-002 §7). `src/team/key.ts` is the
  §4.1 idempotency key in JS; `tests/team-events.test.ts` pins it to the DES vectors and to the keys
  wicked-core pins for its fixtures, and compares the engine's round-trip fixtures, typed against the
  mirror, by value. Types only: nothing relays these yet.
