#!/usr/bin/env python3
"""
studio_standalone_test.py — the task-#84 functional gate: prove the studio SPA is a
FORMALLY INDEPENDENT client of the wicked-crew daemon.

What "independent" means here, and what this script proves end-to-end:

  1. the studio builds ALONE (`npm run -w packages/studio build`) — no crew build is
     invoked anywhere in this flow; the SPA compiles against the shared wire contract
     (`packages/crew-api-types`), not against the daemon's source
  ...
  4. the daemon's loopback CORS admits the second port (the LOOPBACK_ORIGIN regex in
     `server.ts` allows ANY localhost port — verified live, not assumed)
...
The daemon runs from the BUILT package (`node packages/crew/dist/cli/index.js serve`,
i.e. the `wicked-crew` bin target) with `--stub` (deterministic offline engine) and a
throwaway db. Build the daemon first with the plain `npm run -w packages/crew build`
(tsc only, NO studio bundle) — the daemon then serves headless API+WS, which is the
point: the UI the browser loads can only have come from the static server.

Prereqs:  daemon built (plain build), Python Playwright (`pip install playwright &&
playwright install chromium`). The script builds the studio itself unless
SKIP_STUDIO_BUILD=1 (in which case packages/studio/dist must already be baked for
CREW_PORT).

Env knobs: CREW_PORT (default 7901), STUDIO_PORT (default 4310), SKIP_STUDIO_BUILD.

Prints a JSON report to stdout (exit 0/1); screenshots land in e2e/shots/.
Operator-run smoke, same convention as the other scripts in e2e/ — though this one
spends no tokens (stub engine, no real CLI seats).
"""
...
if os.environ.get("SKIP_STUDIO_BUILD") != "1":
    ...
    report["steps"]["studio_build"] = {
        "ok": True,
        "cmd": "npm run -w packages/studio build",
        "vite_api_host": f"127.0.0.1:{CREW_PORT}",
        "crew_build_invoked": False,
    }

dist = REPO / "packages" / "studio" / "dist"
...
# ── 3. Daemon from the built package (bin target), stub engine, throwaway db ──
cli = REPO / "packages" / "crew" / "dist" / "cli" / "index.js"
if not cli.is_file():
    fail("daemon", f"{cli} missing — build the daemon first (npm run -w packages/crew build)")
headless = not (REPO / "packages" / "crew" / "dist" / "studio" / "index.html").is_file()
tmp = mkdtemp(prefix="studio-standalone-")
daemon = subprocess.Popen(
    ["node", str(cli), "serve", "--port", str(CREW_PORT), "--db", os.path.join(tmp, "core.db"), "--stub"],
    cwd=REPO, env=dict(os.environ, WICKED_MEMORY_EMBEDDER="hash"),
...
try:
    ...
    # ── 5. Launch a run over REST (human gate ⇒ parks deterministically) ──────
    problem = "task-84 standalone-studio smoke: prove the SPA drives the daemon cross-origin"
    ...
    # ── 6. Real browser: list runs → open run → approve gate → watch WS ───────
    try:
