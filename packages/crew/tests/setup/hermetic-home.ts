...
// installed wicked-garden plugin. Un-armed, a test boot would copy the developer's real plugin
// cache into ~/.wicked-crew/skills. Root → temp; source → a path that holds no plugin, so the seed
// says "no source" (fast, no copy) unless a test aims it at a fixture. (The seam never writes into
// the user's CLI directories — design v3.2 §1 — so there is no mirror home to arm;
// tests/skills-no-user-cli-writes.test.ts proves it against a fake HOME.)
