 ...
 * the assist skill's Step 8a scene decomposition, the editorial half) then spec (build,
 * creator role: the Playwright authoring half) — so the spec is written against a stated
 * scene plan instead of one-shotting the brief into a single scene (the exact mistake Step 8a
 * opens with: "Do NOT copy the brief text into a single scene").
 *
 ...
 * wording). So both build phases demand a prose report of what they wrote: the phase's own
 * reviewable substance, not decoration.
 *
  ...
  phases: [
    {
      ...
      kind: 'recon',
      instructions:
        'Plan the demo, do not write the spec yet: read the task (the target application URL and the user\'s brief), then decompose the brief into 3-6 named scenes — each a short capability label stating what the viewer should take away (good: "Create a document"; bad: "Click the New button"), covering setup/context beats, the main value moments, and the payoff. Merge trivial setup steps, split compound flows — a demo that is one scene for a multi-step brief is not a demo. For each scene note the click-path you expect (navigation, selectors, waits) and a one-sentence narration line that states the capability, not the on-screen data. Never invent app features the brief and URL do not support. Output the scene plan as plain text. Do NOT write any files in this phase.',
      gate_type: 'value',
      gate: 'auto',
      executes_code: false,
...
export async function startInteractiveDemoSubscriber(
  ...
  const offCoreEvents = adapter.onEvent((event: CoreEvent) => {
    ...
    if (event.type === 'unitDispatched') {
      ...
      narrate(
        flight,
        flight.leg === 'spec' && ord < flight.phaseCount
          ? `Crew phase ${ord}/${flight.phaseCount}: planning the demo scenes…`
          : `Crew phase ${ord}/${flight.phaseCount}: writing ${authoring}…`,
      );
      return;
    }
