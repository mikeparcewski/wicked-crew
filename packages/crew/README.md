...
inherited `WICKED_ESTATE_DB` is honoured); events the engine cannot store dead-letter to
`<core db>.governance/emit-outbox.ndjson`, never under HOME, are counted on
